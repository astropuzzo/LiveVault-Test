...
(() => {
  ...
  function archiveRecordRow(recording) {
    ...
    return `<article class="rec-card ${reasons.length?'lv-attention':''}" data-recording-id="${recording.id}">
      ...
      <div class="archive-identity"><strong>${creatorLinkMarkup(source?.id||0,creator)}</strong><span title="${esc(recording.filename)}">${esc(recording.filename)}</span><small>${esc(dateText(recording.started_at))}</small>${typeof nsfwBadgeMarkup==='function'?nsfwBadgeMarkup(recording):''}</div>
