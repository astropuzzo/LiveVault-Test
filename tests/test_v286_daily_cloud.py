...
def test_v286_version_and_cache():
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == "3.4.8"
    assert 'VERSION = "3.4.8"' in (ROOT / "app/main.py").read_text(encoding="utf-8")
    assert "livevault-shell-v3.4.8" in (ROOT / "app/static/sw.js").read_text(encoding="utf-8")
