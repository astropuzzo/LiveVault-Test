...
def test_release_tracks_current_version():
    assert (ROOT / "VERSION").read_text(encoding="utf-8").strip() == "3.4.19"
    assert 'VERSION = "3.4.19"' in (ROOT / "app/main.py").read_text(encoding="utf-8")
    assert "livevault-shell-v3.4.19" in (ROOT / "app/static/sw.js").read_text(encoding="utf-8")
