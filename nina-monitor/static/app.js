...
function evidenceMarkup(f, lang = 'en') {
  ...
  const title = rescued ? t('Recuperato','Recovered') : rejected ? t('Scartato','Rejected') : checked ? t('Stelle verificate','Stars verified') : attempted ? t('Prova insufficiente','Insufficient evidence') : t('Controllo non eseguito','Check not run');
  const note = rescued ? (f.starCountFalsePositive ? t('Segnale delle stelle abbinate entro i limiti di recupero.','Matched stellar signal within recovery limits.') : t('Forma stellare entro i limiti di recupero.','Star shapes within rescue limits.')) + (String(f.status).toUpperCase()==='LEARNING' ? t(' Baseline in apprendimento.',' Baseline learning.') : t(' Frame utilizzabile.',' Usable frame.')) : rejected ? (/LOW_SESSION_SIGNAL/.test(f.reason||'') ? t('Segnale stellare sotto il minimo di sessione.','Stellar signal below the session minimum.') : /SKY_SIGNAL_LOSS/.test(f.reason||'') ? t('Calo improvviso del segnale e del numero di stelle.','Sudden loss of stellar signal and star detections.') : /STELLAR_FLUX_LOSS/.test(f.reason||'') ? t('Perdita di segnale stellare oltre il limite.','Measured stellar signal loss exceeds the limit.') : !checked ? t('Analisi stellare non conclusiva; scarto originale mantenuto.','Stellar check inconclusive; original rejection retained.') : compromised ? t('Limite di forma stellare superato.','Star-shape limit exceeded.') : f.imageEvidenceHasRescueMargin === false || /BORDERLINE/i.test(f.secondPassText||'') ? t('Eccentricità troppo vicina al limite: margine di recupero insufficiente.','Eccentricity too close to the limit: insufficient rescue margin.') : t('Limite di guida o segnale ancora superato. Vedi esito registrato.','Guiding or signal limit still exceeded. See recorded result.')) : attempted && !checked ? (f.stellarPhotometryAvailable === true ? t('Forma non verificata. Il segnale stellare è misurato separatamente.','Shape not verified. Stellar signal is measured independently.') : t('Analisi stellare non conclusiva.','Stellar check inconclusive.')) : checked ? ((f.reviewReasons||[]).includes('TRANSPARENCY_CHANGE') ? t('Cielo o segnale cambiati. File conservato per revisione.','Sky or stellar signal changed. Frame kept for review.') : t('Misure disponibili.','Measurements available.')) : t('Nessuna analisi stellare per questo frame.','No stellar analysis for this frame.');
  ...
  const metrics = checked || f.stellarPhotometryAvailable === true ? `<div class="stellar-metrics">${[
    ...
    [t('Età riferimenti','Reference age'),number(f.stellarReferenceAgeMinutes,0)+' min',number(f.stellarReferenceFrames,0)+' '+t('riferimenti puliti','clean references')],
    [t('Segnale / atteso','Signal vs expected'),number(percent(f.relativeStellarFlux),0)+'%',t('100% = livello recente atteso','100% = recent expected level')],
    [t('Segnale di sessione','Session signal'),number(percent(f.sessionRelativeStellarFlux),0)+'%',t('minimo','minimum')+' '+number(f.minimumSessionSignalPercent,0)+'%'],
    [t('Andamento del segnale','Signal trend'),f.stellarSignalTrendUsed ? number(f.stellarSignalTrendPercentPerHour,1)+'%/h' : '—',number(f.matchedStars,0)+' '+t('stelle abbinate','matched stars')],
